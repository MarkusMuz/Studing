...
<html>
    ...
    <body>
        ...
        <p>The challenges for scalable fonts are the support-software overhead and the hinting required for more compact renderings. Some fonts are designed for high resolution or print applications, and their hinting doesn’t necessarily translate into efficient and good-looking fonts on a lower-resolution display. 
            ...
            Support for the iType Spark and WorldType Shaper Spark can easily translate onto other platforms and displays.</p>
        
        <p>Визначити загальних кількість слів в тексті: <%=request.getAttribute("task2")%></p>        
        <p>Визначити кількість унікальних слів в тексті: <%=request.getAttribute("task3")%></p>        
        <p>Визначити перші 3 слів, що зустрічаються найчастіше: <%=request.getAttribute("task4")%></p>        
        <p>Визначити кількість слів що не містять літеру p: <%=request.getAttribute("task5")%></p>        
        <p>Визначити кількість слів, що мають рівно 2 однакових літер: <%=request.getAttribute("task6")%></p>        
        <p>Визначити перші 3 трьохлітерні послідовності у словах тексту, що зустрічаються найчастіше: <%=request.getAttribute("task7")%></p>
    </body>
</html>
